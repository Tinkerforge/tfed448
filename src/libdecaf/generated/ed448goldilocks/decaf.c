...
decaf_error_t API_NS(base_double_scalarmul_non_secret) (
    point_t combo,
    const scalar_t scalar1,
    const point_t base2,
    const scalar_t scalar2
...
#if defined(ESP_PLATFORM)
    struct verify_scratch *scratch = heap_caps_aligned_alloc(
        _Alignof(struct verify_scratch), sizeof(struct verify_scratch),
        MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    if (scratch == NULL) {
        return DECAF_OUT_OF_MEMORY;
    }
#else
    struct verify_scratch scratch_storage;
    struct verify_scratch *scratch = &scratch_storage;
#endif
    struct smvt_control *control_var = scratch->control_var;
    struct smvt_control *control_pre = scratch->control_pre;
    pniels_t *precmp_var = scratch->precmp_var;
...
cleanup:
    decaf_bzero(scratch, sizeof(*scratch));
#if defined(ESP_PLATFORM)
