...
static void decaf_ed448_sign_internal (
...
void decaf_ed448_sign (
    ...
    decaf_ed448_derive_public_key(rederived_pubkey, privkey);
    if (DECAF_TRUE != decaf_memeq(rederived_pubkey, pubkey, sizeof(rederived_pubkey))) {
        abort();
    }
    decaf_ed448_sign_internal(signature,privkey,rederived_pubkey,message,
...
void decaf_ed448_sign_prehash (
    ...
    decaf_ed448_sign_internal(signature,privkey,rederived_pubkey,hash_output,
        sizeof(hash_output),1,context,context_len);
    decaf_bzero(hash_output,sizeof(hash_output));
}

void decaf_ed448_derive_keypair (
    decaf_eddsa_448_keypair_t keypair,
    const uint8_t privkey[DECAF_EDDSA_448_PRIVATE_BYTES]
) {
    memcpy(keypair->privkey, privkey, DECAF_EDDSA_448_PRIVATE_BYTES);
    decaf_ed448_derive_public_key(keypair->pubkey, keypair->privkey);
}

void decaf_ed448_keypair_extract_public_key (
    uint8_t pubkey[DECAF_EDDSA_448_PUBLIC_BYTES],
    const decaf_eddsa_448_keypair_t keypair
) {
    memcpy(pubkey,keypair->pubkey,DECAF_EDDSA_448_PUBLIC_BYTES);
}

void decaf_ed448_keypair_extract_private_key (
    uint8_t privkey[DECAF_EDDSA_448_PRIVATE_BYTES],
    const decaf_eddsa_448_keypair_t keypair
) {
    memcpy(privkey,keypair->privkey,DECAF_EDDSA_448_PUBLIC_BYTES);
}

void decaf_ed448_keypair_destroy (
    decaf_eddsa_448_keypair_t keypair
) {
    decaf_bzero(keypair, sizeof(decaf_eddsa_448_keypair_t));
}

void decaf_ed448_keypair_sign (
    uint8_t signature[DECAF_EDDSA_448_SIGNATURE_BYTES],
    const decaf_eddsa_448_keypair_t keypair,
    const uint8_t *message,
    size_t message_len,
    ...
    decaf_ed448_sign_internal(signature,keypair->privkey,keypair->pubkey,message,
        message_len,prehashed,context,context_len);
}

void decaf_ed448_keypair_sign_prehash (
    uint8_t signature[DECAF_EDDSA_448_SIGNATURE_BYTES],
    const decaf_eddsa_448_keypair_t keypair,
    const decaf_ed448_prehash_ctx_t hash,
    const uint8_t *context,
    ...
    decaf_ed448_sign_internal(signature,keypair->privkey,keypair->pubkey,hash_output,
        sizeof(hash_output),1,context,context_len);
    decaf_bzero(hash_output,sizeof(hash_output));
}
...
decaf_error_t decaf_ed448_verify (
...
#if DECAF_448_SCALAR_BYTES < DECAF_EDDSA_448_PRIVATE_BYTES
    for (unsigned i = DECAF_448_SCALAR_BYTES;
         i < DECAF_EDDSA_448_PRIVATE_BYTES;
         i++) {
        if (signature[DECAF_EDDSA_448_PUBLIC_BYTES+i] != 0x00) {
    ...
    error = API_NS(base_double_scalarmul_non_secret)(
        pk_point,
        response_scalar,
        pk_point,
        challenge_scalar
